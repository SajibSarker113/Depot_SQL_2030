...
FROM SCENT.BILL_MAS
WHERE SC_CD='&&SC_CD'
  AND DELI_DT BETWEEN TO_DATE ('01FEB2024','DDMMYYYY') AND TO_DATE ('28FEB2024','DDMMYYYY')
  AND SALE_CD IN ('IC','IM','IN') 
GROUP BY FL_MVH, DIST_CD, THA_CD, UN_CD, CL_ID
UNION
...
WHERE A.SC_CD='&&SC_CD' 
  ...
  AND A.BILL_NO=B.BILL_NO
  AND B.CANCL IS NULL
  AND B.RTN_DT BETWEEN TO_DATE ('01FEB2024','DDMMYYYY') AND TO_DATE ('28FEB2024','DDMMYYYY')
  AND A.SALE_CD IN ('IC','IM','IN') 
GROUP BY A.FL_MVH, A.DIST_CD, A.THA_CD, A.UN_CD, A.CL_ID
UNION
